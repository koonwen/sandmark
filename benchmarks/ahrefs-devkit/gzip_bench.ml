...
(* Benchmark runner *)
let run_benchmark ~config ~test_fn () =
  (* Warmup *)
...
let run_all_benchmarks ?(config = default_config) () =
  bench_small_buffer_storm config;
  bench_large_block_compression config;
  bench_streaming_operations config;
  bench_mixed_size_patterns config;
  bench_concurrent_style config;
  bench_headers_metadata config;
  bench_buffer_recycling config;
  bench_compression_pipelines config
