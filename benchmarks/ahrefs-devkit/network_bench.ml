...
let run_all_benchmarks ?(config = default_config) () =
  bench_ipv4_parsing_storm config;
  bench_cidr_calculations config;
  bench_range_operations config;
  bench_mixed_format_parsing config;
  bench_nat_tables config;
  bench_ip_sorting config;
  bench_broadcast_calculations config;
  bench_complex_network_ops config
